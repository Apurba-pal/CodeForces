...
# Server time: Sep/22/2026 21:44:53UTC+5.5 (k3).
...
l = int(input())
arr = [(int(input()), input(), input()) for _ in range(l)]
...
for n , a, b in arr:
    ...
    a_even = sum(1 for i in range(n) if i & 1 == 0 and a[i] == '1')
    a_odd = sum(1 for i in range(n) if i & 1 != 0 and a[i] == '1')

    b_even = sum(1 for i in range(n) if i & 1 == 0 and b[i] == '1')
    b_odd = sum(1 for i in range(n) if i & 1 != 0 and b[i] == '1')

    if a_even == b_even and b_odd == a_odd:
        print("YES")
    else:
        print("NO")